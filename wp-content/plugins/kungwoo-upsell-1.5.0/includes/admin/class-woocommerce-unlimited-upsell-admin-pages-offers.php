...
class WooCommerce_Unlimited_Upsell_Admin_Pages_Offers
{
            ...
            $js_options = array( 
              ...
              'e' =>
                array(
                  ...
                  'search_for_category' => esc_attr__('Search for a category…', 'woocommerce-unlimited-upsell'),
    ...
    public function display_plugin_page_offer_edit()
    {
        ...
        //  check_ajax_referer('nonce_id', 'nonce');
          ...
          $conditions = isset($_POST['conditions']) ? serialize($_POST['conditions']) : '';
          ...
          if ($id > 0){
            ...
            $url = admin_url('admin.php?page=' . $this->plugin_slug . '-offer-edit' . '&action=edit' . '&id=' . $id . '&msg=updated');
            wp_redirect($url); 
          }else{
            // Add new offer
            $wpdb->insert(
            ...
            // Redirect to offer-edit page with offer id
            $id = $wpdb->insert_id;
            $url = admin_url('admin.php?page=' . $this->plugin_slug . '-offer-edit' . '&action=edit' . '&id=' . $id . '&msg=created');
            wp_redirect($url); 
          }
        }

        if (isset($_GET['id']) && $_GET['id'] != 0) {
            $offer = $wpdb->get_row('SELECT * FROM ' . WooCommerce_Unlimited_Upsell_DB::get_table_name('offers') . ' WHERE id = ' . $_GET['id']);
