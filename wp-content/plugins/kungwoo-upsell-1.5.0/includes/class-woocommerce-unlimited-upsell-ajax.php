...
class WooCommerce_Unlimited_Upsell_AJAX
{
    ...
    private function __construct()
    {
        ...
        if (current_user_can('manage_options')) {
            ...
            add_action('wp_ajax_wooup_admin_search_categories', array($this, 'ajax_search_categories'));
    ...
    public function ajax_search_products()
    {
        ...
        // Security check
        check_ajax_referer('wooup_admin_ajax_request', 'nonce');
    ...
    public function ajax_search_categories()
    {
        ...
        $result = array();
        $search = empty($_REQUEST['q']) ? '' : trim($_REQUEST['q']);
        $args = array(
            'taxonomy' => 'product_cat',
            'hide_empty' => false,
            'name__like' => $search,
            'suppress_filter' => true,
            'number' => 500, // let's have a sane limit here.
        );

        // This is called by select2 when document is ready
        // so the code can add names that are linked to IDs.
        // The params passes comma separated IDs and we'll find their names.
        if (!empty($_REQUEST['post_ids'])) {
            $post_ids = preg_split('#\s*\,\s*#si', $_REQUEST['post_ids']);
    ...
    public function ajax_get_widget_data()
    {
        ...
        switch($widget_type){
          case 'most_triggered_offers':
            ...
            $sql = 'SELECT `name`, `offer_id`, count(`offer_id`) as `count` FROM `' . $table_name_1 . '`, `' . $table_name_2 . '` WHERE `' . $table_name_1 . '`.`id` = `' . $table_name_2 . '`.`offer_id` GROUP BY `offer_id` ORDER BY `count` DESC LIMIT 5';
          ...
          case 'most_popular_upsell_products':
            ...
            $sql = 'SELECT `product_id`, (count(`product_id`)*`product_qty`) as `count` FROM `' . $table_name . '` GROUP BY `product_id` ORDER BY `count` DESC  LIMIT 5 ';
            $widget_data = $wpdb->get_results( $sql, ARRAY_A );
    ...
    public function ajax_add_to_cart()
    {
        ...
        // Security check
        check_ajax_referer('wooup_ajax_request', 'nonce');
