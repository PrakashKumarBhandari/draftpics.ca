...
class WooCommerce_Unlimited_Upsell
{
                ...
                if(isset($condition['status']) && $condition['status']==1){
                  switch($index){
                    ...
                    case 2: // Condition #3 (Specific products are in cart)
                      $products = $this->parse_csv_line(empty($condition['products']) ? '' : $condition['products']);
                        
                      if ($condition['relation'] == 'any') {
                        $cart_items_intersect = array_intersect($cart_items_ids, $products);

                        // We need at least 1 item.
                        if (!empty($cart_items_intersect)) {
                          $conditions_passed++;
                        }
                      } else { // all
                          $diff = array_diff($cart_items_ids, $products);

                          // the arrays should be a complete match
                          if (empty($diff) || $cart_items_ids == $products) {
                            $conditions_passed++;
                          }
                      }
                    break;
                    ...
                    case 4: // Condition #4 (Specific categories are in cart)
                       $cat_ids = $this->parse_csv_line(empty($condition['categories']) ? '' : $condition['categories']);
                       $matched = 0;
                       
                       if ($condition['relation'] == 'any') {
                          foreach ($cart_items_ids as $product_id) {
                              $prod_cats_obj_arr = get_the_terms( $product_id, 'product_cat' );
                              
                              // the user only wants one category to match and we're good.
                              // skip the processing to save resources.
                              foreach ($prod_cats_obj_arr as $cat_obj) {
                                 if (in_array($cat_obj->term_id, $cat_ids)) {
                                     $matched = 1;
                                     break;
                                 }
                              }
                          ...
                          // Here we want all the required categories from the conditions to exist in the list.
                          // the products can belong to multiple categories and those will be added to the list.
                          ...
                          foreach ($cart_items_ids as $product_id) {
                            ...
                            foreach ($prod_cats_obj_arr as $cat_obj) {
                                $all_product_cats[] = $cat_obj->term_id;
                            }
                            
                            $all_product_cats = array_unique($all_product_cats);

                            // Let's see which categories match
                            $intersection = array_intersect($cat_ids, $all_product_cats);

                            // the arrays should be a complete match
                            // as soon as we detect all the category match we exit the loop.
                            if ($intersection == $cat_ids) {
                                $matched = 1;
                                break;
                            }
                          } // foreach cart items
    ...
    public function parse_csv_line($product_list) {
        $products = empty($product_list) ? array() : explode(',', $product_list);
    ...
    private function show_offer($offer_id = 0)
    {
        ...
        // Select offer
        $offer = $wpdb->get_row( 'SELECT * FROM ' . WooCommerce_Unlimited_Upsell_DB::get_table_name('offers') . ' WHERE (`is_active`=1) AND (`id`= ' . $offer_id . ')LIMIT 1', OBJECT );

        // List of offer products
        
        $offer_items       = $this->parse_csv_line(empty($offer->products) ? '' : $offer->products);
