...
vc_map( array(
	'name'                    => esc_html__( 'Exhibition Portfolio', 'js_composer' ),
	'base'                    => 'wiso_exhibition',
...
class WPBakeryShortCode_wiso_exhibition extends WPBakeryShortCode {
	...
	protected function content( $atts, $content = null ) {
		...
		if ( !in_array( "wiso_exhibition-css", self::$css_scripts ) ) {
			self::$css_scripts[] = "wiso_exhibition-css";
		}
		$this->enqueueCss();


		if ( !in_array( "wiso_exhibition", self::$js_scripts ) ) {
			self::$js_scripts[] = "wiso_exhibition";
		}
                    ...
                    <div class="scroller">
                        <?php while ( $posts->have_posts() ) :
                            ...
                            $slideContent .= '<div class="slide">
                                                <span class="cat">'. get_the_term_list($posts->ID, "portfolio-category", '', ', ') . '</span>
                                                    <h2 class="slide__name">' . get_the_title() . '</h2>
                                                    <h3 class="slide__title">
                                                        <div>' . get_the_excerpt() . '
                                                            <span>
                                                                <a href="'. esc_url($link) .'" class="a-btn-2 slide__number" target="' . esc_attr($target) . '">' . esc_html($btn_text) . '</a>
                                                            </span>
                                                        </div>
                                                    </h3>
