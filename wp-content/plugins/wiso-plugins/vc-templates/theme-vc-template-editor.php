...
/**
 * Class Wiso_Vc_Templates_Editor
 * @since 1.0
 */
class Wiso_Vc_Templates_Editor {
    /**
     ...
     * @var bool
     */
    protected $wiso_templates = false;
    /**
     * @since 4.4
    ...
    public function addTemplatesTab( $data ) {
        $newCategory = array(
            'category'        => 'wiso_templates',
            'category_name'   => esc_html__( 'Wiso Templates', 'wiso' ),
            'category_weight' => 1,
    ...
    protected function get_template_categories() {
        ...
        $categories = array(
            'all'         => esc_html__( 'All', 'wiso' ),
            'about'      => esc_html__( 'About', 'wiso' ),
            'banner'      => esc_html__( 'Banners', 'wiso' ),
            'buttons'    => esc_html__( 'Buttons', 'wiso' ),
            'clients'         => esc_html__( 'Clients', 'wiso' ),
            'coming-soon'         => esc_html__( 'Coming soon', 'wiso' ),
            'contacts'    => esc_html__( 'Contacts', 'wiso' ),
            'events'     => esc_html__( 'Events', 'wiso' ),
            'gallery'     => esc_html__( 'Gallery', 'wiso' ),
            'headings'        => esc_html__( 'Headings', 'wiso' ),
            'instagram'        => esc_html__( 'Instagram', 'wiso' ),
            'line-of-images'        => esc_html__( 'Line of images', 'wiso' ),
            'pages'        => esc_html__( 'Pages', 'wiso' ),
            'portfolio'        => esc_html__( 'Portfolio', 'wiso' ),
            'posts'        => esc_html__( 'Posts', 'wiso' ),
            'services'     => esc_html__( 'Services', 'wiso' ),
            'skills'        => esc_html__( 'Skills', 'wiso' ),
            'teams'        => esc_html__( 'Team', 'wiso' ),
            'pricing'     => esc_html__( 'Pricing', 'wiso' ),
            'testimonial' => esc_html__( 'Testimonials', 'wiso' ),
        );
    ...
    public function renderTemplateBlock( $category ) {
        ...
        if ( 'wiso_templates' === $category['category'] ) {

            $category['output'] = '<div class="vc_col-md-2 wiso-sorting-container">';
            $category['output'] .= $this->get_template_categories();
            $category['output'] .= '</div>';


            $category['output'] .= '
			<div class="vc_column vc_col-sm-12 wiso-templates-container">
    ...
    function renderTemplateWindow( $template_name, $template_data ) {
        if ( 'wiso_templates' === $template_data['type'] ) {
            return $this->renderTemplateWindowWisoTemplates( $template_name, $template_data );
        }
     ...
     * @return string
     */
    public function renderTemplateWindowWisoTemplates( $template_name, $template_data ) {
        ob_start();
        $template_id = esc_attr( $template_data['unique_id'] );
        $template_id_hash = md5( $template_id ); // needed for jquery target for TTA
        $template_name = wp_kses_post( $template_name );
        $preview_template_title = esc_attr( 'Preview template', 'wiso' );
        $add_template_title = esc_attr( 'Add template', 'wiso' );

        echo <<<HTML
		<span class="vc_ui-list-bar-item-trigger" title="$add_template_title"
			data-template-handler=""
			data-vc-ui-element="template-title">$template_name</span>
...
HTML;
    ...
    public function renderBackendTemplate() {
        ...
        if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
            die( 'Error: Vc_Wiso_Templates::renderBackendTemplate:1' );
        }
    ...
    public function getAllTemplates() {
        ...
        $data = array();
        $wiso_templates = $this->getTemplates();
        $category_templates = array();
        foreach ( $wiso_templates as $template_id => $template_data ) {
            $category_templates[] = array(
    ...
    public function loadDefaultTemplates() {
        ...
        if ( ! is_array( $this->wiso_templates ) ) {
            $this->wiso_templates = $this->allTemplates();
        }

        return $this->wiso_templates;
    }
    ...
    public function getDefaultTemplate( $template_index ) {
        ...
        if ( ! is_numeric( $template_index ) || ! is_array( $this->wiso_templates ) || ! isset( $this->wiso_templates[ $template_index ] ) ) {
            return false;
        }

        return $this->wiso_templates[ $template_index ];
    }
    ...
    public function addDefaultTemplates( $data ) {
        if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
            if ( ! is_array( $this->wiso_templates ) ) {
                $this->wiso_templates = array();
            }
            $this->wiso_templates[] = $data;
    ...
    public function renderTemplateListItem( $template ) {
        $name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'wiso' ) );
...
HTML;
        $output .= '<div class="wiso-template-preview">';
        if ( $new ) { $output .= '<span class="wiso-badge-new">New</span>'; }
        $output .= '<img src="' . esc_url( $template_image ) . '" alt="' . esc_attr( $name ) . '" width="300" height="200" /></div>';
        $output .= apply_filters( 'vc_templates_render_template', $name, $template );
        $output .= '<span class="wiso-template-categories">' . esc_html( $template_sort_name ) . '</span>';
        $output .= <<<HTML
						</div>
