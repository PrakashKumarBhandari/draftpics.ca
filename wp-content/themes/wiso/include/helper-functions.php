...
if ( ! function_exists( 'wiso_add_default_cs_websafe_fonts' ) ) {
	function wiso_add_default_cs_websafe_fonts( $params ) {
		return array_merge( array( '' ), $params );
	}
}
add_filter( 'cs_websafe_fonts', 'wiso_add_default_cs_websafe_fonts' );

/**
 * Сustom wiso menu.
 */
if ( ! function_exists( 'wiso_custom_menu' ) ) {
	function wiso_custom_menu() {
		...
		$portfolio_data = get_post_meta( get_the_ID(), 'wiso_portfolio_options', true );
		$events_data    = get_post_meta( get_the_ID(), 'wiso_events_options', true );
		...
		if ( isset( $meta_data['page_menu'] ) && $meta_data['page_menu'] !== 'none' ) {
			...
			if ( has_nav_menu( 'primary-menu' ) ) {
				...
				echo '<span class="no-menu">' . esc_html__( 'Please register Top Navigation from', 'wiso' ) . ' <a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" target="_blank">' . esc_html__( 'Appearance &gt; Menus', 'wiso' ) . '</a></span>';
			}
		}
 ...
 *
 */
if ( ! function_exists( 'wiso_get_first_url_from_string' ) ) {
	function wiso_get_first_url_from_string( $string ) {
 ...
 *
 */
if ( ! function_exists( 'wiso_get_shortcode_regex' ) ) {
	function wiso_get_shortcode_regex( $tagregexp = '' ) {
 ...
 *
 */
if ( ! function_exists( 'wiso_tagregexp' ) ) {
	function wiso_tagregexp() {
		return apply_filters( 'wiso_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|cs_media' );
	}
}
 ...
 *
 */
if ( ! function_exists( 'wiso_comment' ) ) {
	function wiso_comment( $comment, $args, $depth ) {
		...
		switch ( $comment->comment_type ):
			...
			case 'trackback': ?>
                <p>
					<span class="pin-title"><?php esc_html_e( 'Pingback:', 'wiso' ); ?></span><?php comment_author_link(); ?>
					<?php edit_comment_link( esc_html__( '(Edit)', 'wiso' ), '<span class="edit-link">', '</span>' ); ?>
                </p>
				<?php
				break;
			default:
                    ...
                    <div class="content">
                        ...
                        <div class="comment-content">
							...
							comment_reply_link(
								array_merge( $args,
									array(
										'reply_text' => esc_html__( 'Reply', 'wiso' ),
										'after'      => '',
										'depth'      => $depth,
 ...
 * Site logo function.
 */
if ( ! function_exists( 'wiso_site_logo' ) ) {
	function wiso_site_logo() {
		?>
        <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="logo">
			...
			if ( is_404() && cs_get_option( 'error_logo' ) ) {
				if ( apply_filters( 'wiso_type_logo', cs_get_option( 'error_site_logo' ) ) == 'txtlogo' ) {
				...
				$meta_data           = get_post_meta( get_the_ID(), '_custom_page_options', true );
				$meta_data_portfolio = get_post_meta( get_the_ID(), 'wiso_portfolio_options', true );
				...
				if ( cs_get_option( 'site_logo' ) == 'imglogo' || $img_logo) {
					...
					$image_logo        = apply_filters( 'wiso_header_logo', $logo );

					$img_src           = ! empty( $image_logo ) ? $image_logo : WISO_T_URI . '/assets/images/logo.png';
 ...
 * Blog item header.
 */
if ( ! function_exists( 'wiso_blog_item_hedeader' ) ) {

	function wiso_blog_item_hedeader( $option, $post_id, $video_params = array(), $classButton = '', $classWrap = '' ) {
		...
		if ( isset( $option[0]['post_preview_style'] ) ) {
			switch ( $option[0]['post_preview_style'] ) {
				case 'image':
					...
					$blog_type = apply_filters( 'wiso_blog_style', $blog_type );
					$imgClass  = $blog_type == 'masonry' ? '' : 's-img-switch';
					$output    = '';
					if ( ! empty( $image ) && ( $format != 'quote' ) ) {
						$output .= '<div class="post-media">';
						$output .= wiso_the_lazy_load_flter( $image[0], array(
							'class' => $imgClass,
							'alt'   => ''
						) );
						$output .= '</div>';
				...
				case 'video':
					...
					$output    .= wiso_the_lazy_load_flter( $video_img[0], array(
						'class' => 's-img-switch',
						'alt'   => ''
					) );
				...
				case 'slider':
					...
					foreach ( $images as $image ) {
						...
						if ( ! empty( $url ) ) {
							...
							$output .= wiso_the_lazy_load_flter( $url, array(
								'class' => 's-img-switch',
								'alt'   => ''
							) );
							$output .= '</li>';
			...
			if ( $format == 'quote' ) {
				...
				if ( ! empty( $image ) ) {
					$output .= '<div class="post-media">';
					$output .= wiso_the_lazy_load_flter( $image, array(
						'class' => $imgClass,
						'alt'   => ''
					) );
					$output .= '</div>';
...
* Get Page For Navi
*/
if ( ! function_exists( 'wiso_get_pages_for_navi' ) ) {
	function wiso_get_pages_for_navi() {
...
if ( ! function_exists( 'wiso_dynamic_css' ) ) {
	function wiso_dynamic_css() {
		require_once get_template_directory() . '/assets/css/custom.css.php';
		wp_die();
	}
}
add_action( 'wp_ajax_nopriv_wiso_dynamic_css', 'wiso_dynamic_css' );
add_action( 'wp_ajax_wiso_dynamic_css', 'wiso_dynamic_css' );

/*
* wiso Mini Cart for Woocommerce
*/
if ( ! function_exists( 'wiso_mini_cart' ) ) {
	function wiso_mini_cart() {
            ...
            <div class="wiso_mini_cart">
                ...
                <ul class="cart_list product_list_widget">
							...
							if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
                                ...
                                <li class="<?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
									...
									echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
										...
										esc_html__( 'Remove this item', 'wiso' ),
										esc_attr( $product_id ),
					...
					<?php else : ?>
                        ...
                        <li class="empty"><?php esc_html_e( 'No products in the cart.', 'wiso' ); ?></li>
                ...
                <div class="wiso-buttons">
                    <a href="<?php echo esc_url(wc_get_cart_url()); ?>"><?php esc_html_e( 'Go to cart', 'wiso' ); ?><i
                                class="fa fa-arrow-right"></i></a>
                    <p class="woocommerce-mini-cart__total total"><?php esc_html_e( 'Total', 'wiso' ); ?>
				...
				<?php if ( ! WC()->cart->is_empty() ) : ?>
                    ...
                    <a href="<?php echo esc_url( wc_get_checkout_url() ); ?>"
                       class="button checkout wc-forward"><?php esc_html_e( 'Checkout', 'wiso' ); ?></a>
...
if ( ! function_exists( 'wiso_the_share_posts' ) ) {
	function wiso_the_share_posts( $post, $show_title = '' ) {
                ...
                <div class="ft-part margin-lg-15b">
                    <ul class="social-list">
						<?php if ( ! empty( $show_title ) ) { ?>
                            <li><span><?php esc_html_e( 'Share:', 'wiso' ); ?></span></li>
						<?php } ?>
                        <li>
...
if ( ! function_exists( 'wiso_include_fonts' ) ) {
	function wiso_include_fonts( $fonts = '' ) {
		if ( empty( $fonts ) ) {
			return '';
		}
...
if ( ! function_exists( 'wiso_maxsite_str_word' ) ) {
	function wiso_maxsite_str_word( $text, $counttext = 10, $sep = ' ' ) {
...
if ( ! function_exists( 'wiso_excerpt_more' ) ) {
	function wiso_excerpt_more() {
		return ' ...';
	}

	add_filter( 'excerpt_more', 'wiso_excerpt_more' );
}
...
if ( ! function_exists( 'wiso_search_popup' ) ) {
	function wiso_search_popup() { ?>
                            ...
                            <form role="search" method="get" class="search-form" action="<?php echo esc_url(home_url( '/' )); ?>">
                                <div class="input-group">
                                    <input type="search" value="<?php echo get_search_query() ?>" name="s"
                                           class="search-field"
                                           placeholder="<?php esc_attr_e( 'Search...', 'wiso' ); ?>"
                                           required>
                                </div>
                            </form>
                        </div>
...
add_action( 'wiso_after_footer', 'wiso_search_popup', 10 );
 ...
 *
 **/
if ( ! function_exists( 'wiso_like_post' ) ) {
	function wiso_like_post() {
		if ( empty( $_POST ) ) {
			esc_html_e( 'Ajax error', 'wiso' );
		} else {
...
add_action( 'wp_ajax_wiso_like_post', 'wiso_like_post' );
add_action( 'wp_ajax_nopriv_wiso_like_post', 'wiso_like_post' );


/**
 * Like counter.
 */
if ( ! function_exists( 'wiso_like_counter' ) ) {
	function wiso_like_counter() {
		// Count post likes
            ...
            <div class="likes-wrap">
                ...
                <span>
                    <?php if($count_post_likes > 1 && !is_single()){ ?>
                        <i class="count"><?php echo esc_html( $count_post_likes . esc_html__(' Likes', 'wiso') ); ?></i>
                    <?php }elseif(!is_single()){ ?>
                        <i class="count"><?php echo esc_html( $count_post_likes . esc_html__(' Like', 'wiso')); ?></i>
                    <?php }else{ ?>
 ...
 * Like button.
 */
if ( ! function_exists( 'wiso_like_button' ) ) {
	function wiso_like_button() { ?>
 ...
 *
 */
if ( ! function_exists( 'wiso_show_extra_profile_fields' ) ) {
	function wiso_show_extra_profile_fields( $user ) { ?>

        <h3><?php esc_html_e( 'Extra profile information', 'wiso' ); ?></h3>

        <table class="form-table">

            <tr>
                <th><label for="twitter"><?php esc_html_e( 'Twitter', 'wiso' ); ?></label></th>

                <td>
                    <input type="text" name="twitter" id="twitter"
                           value="<?php echo esc_url( get_user_meta( $user->ID, 'twitter', true ) ); ?>"
                           class="regular-text"/><br/>
                    <span class="description"><?php esc_html_e( 'Please enter your Twitter profile link.', 'wiso' ); ?></span>
                </td>
            </tr>
            <tr>
                <th><label for="facebook"><?php esc_html_e( 'Facebook', 'wiso' ); ?></label></th>

                <td>
                    <input type="text" name="facebook" id="facebook"
                           value="<?php echo esc_attr( get_user_meta( $user->ID, 'facebook', true ) ); ?>"
                           class="regular-text"/><br/>
                    <span class="description"><?php esc_html_e( 'Please enter your Facebook profile link.', 'wiso' ); ?></span>
                </td>
            </tr>
            <tr>
                <th><label for="instagram"><?php esc_html_e( 'Instagram', 'wiso' ); ?></label></th>

                <td>
                    <input type="text" name="instagram" id="instagram"
                           value="<?php echo esc_attr( get_user_meta( $user->ID, 'instagram', true ) ); ?>"
                           class="regular-text"/><br/>
                    <span class="description"><?php esc_html_e( 'Please enter your Instagram profile link.', 'wiso' ); ?></span>
                </td>
            </tr>
            <tr>
                <th><label for="dribbble"><?php esc_html_e( 'Dribbble', 'wiso' ); ?></label></th>

                <td>
                    <input type="text" name="dribbble" id="dribbble"
                           value="<?php echo esc_attr( get_user_meta( $user->ID, 'dribbble', true ) ); ?>"
                           class="regular-text"/><br/>
                    <span class="description"><?php esc_html_e( 'Please enter your Dribbble profile link.', 'wiso' ); ?></span>
                </td>
            </tr>
...
add_action( 'show_user_profile', 'wiso_show_extra_profile_fields' );
add_action( 'edit_user_profile', 'wiso_show_extra_profile_fields' );
 ...
 *
 */
if ( ! function_exists( 'wiso_save_extra_profile_fields' ) ) {
	function wiso_save_extra_profile_fields( $user_id ) {
...
add_action( 'personal_options_update', 'wiso_save_extra_profile_fields' );
add_action( 'edit_user_profile_update', 'wiso_save_extra_profile_fields' );
...
if ( ! function_exists( 'wiso_fixed_header' ) ) {
	function wiso_fixed_header( $menu_main_style, $post_id ) {
		...
		$fixed_menu_class = apply_filters( 'wiso_blog_menu_style', $fixed_menu_class );
...
if ( ! function_exists( 'wiso_main_header_html' ) ) {
	function wiso_main_header_html() {
...
add_action( 'wiso_main_header', 'wiso_main_header_html' );
