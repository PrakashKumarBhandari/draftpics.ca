...
$post_meta = get_post_meta( $post->ID, 'wiso_post_options' );
...
while ( have_posts() ) :
    ...
    <div class="container">
        <div class="row">
          ...
          <div class="post-details no-padd-md <?php echo esc_attr( $unitClass . ' ' . $preview  . ' ' . $content_size_class ); ?>">
                <div class="single-content <?php echo esc_attr( $protected . $singleClass); ?>">
                <div class="single-content-wrapper">

                    <div class="main-top-content">
                            <?php if ( ! function_exists( 'cs_framework_init' ) || cs_get_option( 'wiso_post_cat' ) ) { ?>
                                <div class="single-categories"><?php the_category( ' ' ); ?></div>
                            <?php }
                            the_title( '<h2 class="title">', '</h2>' ); ?>
                        <div class="title-wrap">
                            <?php if (  ! function_exists( 'cs_framework_init' ) || cs_get_option( 'wiso_post_author' ) ) { ?>
                                <div class="author"><?php esc_html_e( 'by ', 'wiso' ); ?>
                ...
                wiso_blog_item_hedeader( $post_meta, $post->ID, $video_params, 'start', '' );
                }
                elseif ( has_post_thumbnail() ) { ?>
                    <div class="post-banner">
                        <?php $tumb_url = get_the_post_thumbnail_url( $post->ID, 'full' );
                            ...
                            <img src="<?php echo esc_url( $tumb_url ); ?>" alt="<?php echo esc_attr($image_alt); ?>" class="">
                    </div>
                <?php }
                    }
                }
                        ...
                        <div class="post-little-banner">
                            <div class="main-top-content text-center">
                                <?php the_title( '<h2 class="title">', '</h2>' ); ?>
                                    <div class="date-post"><?php the_time( get_option( 'date_format' ) ); ?></div>
                                    <?php if ( cs_get_option( 'wiso_post_author' ) ) { ?>
                                    <div class="author">
                                        <?php esc_html_e( 'Written by: ', 'wiso' );
                                        the_author_link(); ?>
                                    </div>
                                <?php } ?>
                            </div>
				...
				wp_link_pages( 'link_before=<span class="pages">&link_after=</span>&before=<div class="post-nav"> <span>' . esc_html__( "Page:", 'wiso' ) . ' </span> &after=</div>' ); ?>

                    <?php if(get_post_type($post->ID ) !== 'whizzy_proof_gallery'){ ?>

                    <div class="post-info">
						<?php if ( ! function_exists( 'cs_framework_init' ) || cs_get_option( 'wiso_post_tags' ) ) { ?>
                            <div class="single-tags"><?php the_tags( '', ' ' ); ?></div>
						<?php } ?>
                         <?php if ( function_exists( 'cs_framework_init' ) && function_exists( 'wiso_like_counter' ) ) { ?>
                            <div class="likes-wrap">
                                <?php wiso_like_button();
                                wiso_like_counter(); ?>
                                <span>
                                    <?php esc_html_e('Like', 'wiso'); ?>
                                </span>
                            </div>
                        <?php } ?>
                    </div>
                ...
                if(get_post_type($post->ID ) !== 'whizzy_proof_gallery'){
                      ...
                      $post_user_name = cs_get_option('wiso_post_user_name');
                      $post_user_nickname = cs_get_option('wiso_post_user_nickname');
               ...
               if ( $post_author_info ) : ?>
                ...
                <section class="post-author">
                    ...
                    <div class="post-author__content">
                        <div class="post-name-wrap">
	                        <?php if ( !$post_user_name && (! empty( $author_first_name ) || ! empty( $author_last_name )) ) : ?>
                                <span class="post-author__title"><?php echo esc_html( $author_first_name . ' ' . $author_last_name ); ?></span>
	                        <?php endif; ?>
	                        <?php if ( !$post_user_nickname && ! empty( $author_nicename ) ) : ?>
				...
				if ( $query->have_posts() ) {
                    ...
                    <div class="recent-post-single clearfix">
                        <div class="row">
                        <div class="recent-title"><?php esc_html_e('Recent posts', 'wiso'); ?></div>
            ...
            <?php
				wp_reset_postdata();
				if ( ! function_exists( 'cs_framework_init' ) || (isset( $post_meta[0]['wiso_navigation_posts'] )&& $post_meta[0]['wiso_navigation_posts'] ) ) { ?>
                    <div class="single-pagination">
						...
						if ( ! empty( $prev_post ) ) : ?>
                            <div class="pag-prev">
                              <span>
	                            <?php esc_html_e( 'Previous post', 'wiso' ); ?>
