...
get_header();
	...
	<?php } ?>
            ...
            <div class="blog <?php echo esc_attr( $content_size_class . ' ' . $blog_type); ?>">
                <?php
					...
					while ( have_posts() ) : the_post();
                        ...
                        $post_options = get_post_meta( get_the_ID(), 'wiso_post_options' );
					    $no_image = '';
                            ...
                            <div <?php post_class( 'post col-xs-12 post-content center-style format-post-' . $format_post . $no_image); ?>>
                                <div class="post-wrap-item <?php echo esc_attr($preview); ?>">
									...
									<?php if ( ((isset( $post_options[0]['post_preview_style'] ) && ($post_options[0]['post_preview_style'] != 'text' && $post_options[0]['post_preview_style'] != 'link' && $post_options[0]['post_preview_style'] != 'quote')) && ( get_post_format( get_the_ID() ) != 'quote' ) && (get_post_format( get_the_ID() ) != 'link' )) || (! function_exists( 'cs_framework_init' ) && ( get_post_format( get_the_ID() ) != 'quote' ) && (get_post_format( get_the_ID() ) != 'link' ))) {
								        wiso_blog_item_hedeader( $post_options, get_the_ID() );
									} ?>
                                    <div class="info-wrap">
                                        <div class="flex-wrap">
										...
										<?php if ( (isset( $post_options[0]['post_preview_style'] ) && ($post_options[0]['post_preview_style'] == 'text' || $post_options[0]['post_preview_style'] == 'link' )) || ( get_post_format( get_the_ID() ) == 'quote' ) || ( get_post_format( get_the_ID() ) == 'link' )) {
                                            ...
                                            <a href="<?php esc_url(the_permalink()); ?>" class="a-btn-3"><?php esc_html_e('view more', 'wiso'); ?></a>
										<?php } ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        elseif($blog_type == 'metro' || $blog_type == 'masonry' ){
						    if($blog_type == 'masonry'){ ?>
						        <div <?php post_class( 'post col-xs-12 metro-style col-sm-6 col-md-' . esc_attr($post_size_class_masonry) . ' col-lg-' .  esc_attr($post_size_class_masonry) . ' format-post-' .  esc_attr($format_post . $no_image)); ?>>
						    <?php }else{ ?>
						        <div <?php post_class( 'post col-xs-12 metro-style col-sm-6 col-md-' .  esc_attr($post_size_class) . ' col-lg-' .  esc_attr($post_size_class_metro) . ' format-post-' .  esc_attr($format_post . $no_image)); ?>>
						    <?php }?>
                                <div class="post-wrap-item <?php echo esc_attr($preview); ?>">
                                    ...
                                    <div class="info-wrap">
                                       ...
                                       if($blog_type != 'masonry'){ ?>
                                          <div class="counters">
                                            ...
                                            <?php if ( function_exists('wiso_like_counter') ) {
                                                wiso_like_counter();
                                            }
                                            ...
                                            <div class="comments">
                                                ...
                                                <?php $comments = get_comments_number( get_the_ID()) == '1' ? esc_html__('Comment', 'wiso') : esc_html__('Comments', 'wiso'); ?>
       ...
       <?php if ( $blog_type == 'metro' ) : ?>
            ...
            <?php echo '<div class="metro-load-more__button js-load-more">' . esc_html__('Load More', 'wiso') . '</div>'; ?>
            </div>
        <?php endif; ?>
    </div>
 <?php else : ?>
