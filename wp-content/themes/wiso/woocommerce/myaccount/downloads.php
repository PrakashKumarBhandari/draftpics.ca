					...
					<td class="<?php echo  $column_id ; ?>" data-title="<?php echo  $column_name ; ?>">
						...
						<?php elseif ( 'download-expires' === $column_id ) : ?>
							<?php if ( ! empty( $download['access_expires'] ) ) : ?>
								<time datetime="<?php echo date( 'Y-m-d', strtotime( $download['access_expires'] ) ); ?>" title="<?php echo  strtotime( $download['access_expires'] ) ; ?>"><?php echo date_i18n( get_option( 'date_format' ), strtotime( $download['access_expires'] ) ); ?></time>
							<?php else : ?>
								<?php esc_html_e( 'Never', 'wiso' ); ?>
							<?php endif; ?>
...
<?php else : ?>
	<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
		...
		<?php esc_html_e( 'no downloads available yet.', 'wiso' ); ?>
	</div>
<?php endif; ?>
