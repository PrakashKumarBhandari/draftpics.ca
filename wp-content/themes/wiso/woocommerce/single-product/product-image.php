...
if ( $product->is_on_sale() ) {
	...
	$label = apply_filters( 'woocommerce_sale_flash', '<span class="onsale">' . esc_html__( 'Sale', 'wiso' ) . '</span>', $post, $product );

}elseif(isset($label_new) && $label_new && !$product->is_on_sale()){
    $label = '<span class="on-new">'. esc_html__( 'New', 'wiso' ) . '</span>';
 } ?>
...
<div class="wiso_images">
	<?php if ( has_post_thumbnail() || count( $gallery ) > 0 ) {
		if ( count( $gallery ) > 0 ) { ?>
                ...
                <ul class="product-gallery-thumbnail-wrap">
                    <?php foreach ( $gallery as $item ) {
	                    ...
	                    $image_alt = get_post_meta( $item, '_wp_attachment_image_alt', true);
                        echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<li><div class="img-wrapper"><img src="%s" class="s-img-switch" alt="'. $image_alt .'" /></div></li>', $image_url, $image_url ), $post->ID );
                    } ?>
                </ul>
		   	<?php
	    } else {
	    ...
	    echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<figure><img src="%s" data-size="600x450" alt="%s" /></figure>', wc_placeholder_img_src(), esc_html__( 'Placeholder', 'wiso' ) ), $post->ID );
	} ?>
